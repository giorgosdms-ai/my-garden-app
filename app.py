      ...
      for day_num in range(1, num_days + 1):
        ...
        if matching_extras:
          for ex in matching_extras:
            st.warning(
                f"⚡ **[ΕΞΤΡΑΔΑΚΙ] {ex['time']} - {ex['title']}**"
                + (f" <br>_Σημείωση: {ex['notes']}_" if ex.get("notes") else ""),
                icon="🚨",
            )
  ...
  # -------------------------------------------------------------
    ...
    with tab1:
      ...
      ex_time = st.time_input(
          "Ώρα:", datetime.strptime("09:00", "%H:%M").time()
      )
      ...
      if st.button("✅ Αποθήκευση Έκτακτου"):
        if ex_title.strip():
          ...
          save_data()
          st.success("Το εξτραδάκι προστέθηκε με επιτυχία!")
          st.rerun()
